...
const Settings: React.FC = () => {
  ...
  const handleCloseYear = async (id: string) => {
      if (!settings || !confirm('آیا از بستن این سال مالی اطمینان دارید؟ اسناد این سال دیگر قابل ویرایش نخواهند بود.')) return;
      
      const updated = {
          ...settings,
          fiscalYears: settings.fiscalYears?.map(y => y.id === id ? { ...y, isClosed: true, endDate: new Date().toISOString() } : y)
      };
            ...
            <form onSubmit={handleSave} className="space-y-8 max-w-4xl mx-auto">
                ...
                {activeCategory === 'fiscal' && (
                    <div className="space-y-8 animate-fade-in">
                        <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
                            <h3 className="font-bold text-blue-800 mb-4 flex items-center gap-2"><Plus size={20}/> افتتاح سال مالی جدید</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                                ...
                                <div><label className="text-xs font-bold text-gray-600 block mb-1">شروع دستور پرداخت</label><input type="number" className="w-full border rounded-lg p-2" value={startPay} onChange={e=>setStartPay(e.target.value)}/></div>
                                <div><label className="text-xs font-bold text-gray-600 block mb-1">شروع مجوز خروج</label><input type="number" className="w-full border rounded-lg p-2" value={startExit} onChange={e=>setStartExit(e.target.value)}/></div>
                                <button type="button" onClick={handleAddFiscalYear} className="bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-blue-700 h-[42px]">ایجاد سال مالی</button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                {settings?.fiscalYears?.map(y => (
                                    <div key={y.id} className={`p-4 rounded-xl border flex flex-col md:flex-row justify-between items-center gap-4 ${y.id === settings.activeFiscalYearId ? 'border-blue-500 bg-blue-50/30 ring-1 ring-blue-500' : 'bg-white border-gray-200'}`}>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {y.isClosed && (
                                                <span className="text-gray-400 text-xs font-bold flex items-center gap-1"><CheckCircle2 size={14}/> این سال مالی بسته شده است</span>
                                            )}
                                        </div>
                                    </div>
                                ))}
                ...
                {activeCategory === 'system' && (
                    <div className="space-y-4">
                        ...
                        <p className="text-sm text-gray-500">سایر تنظیمات سیستم را در این بخش مدیریت کنید.</p>
                    </div>
                )}
