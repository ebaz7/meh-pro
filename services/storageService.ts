...
export const getNextBijakNumber = async (company: string): Promise<number> => { try { const response = await apiCall<{ nextNumber: number }>(`/next-bijak-number?company=${encodeURIComponent(company)}`); return response.nextNumber; } catch (e) { return 1001; } };
