...
const ChatRoom: React.FC<ChatRoomProps> = ({ currentUser, onNotification }) => {
    ...
    const loadData = async () => {
        try {
            ...
            // Notification Logic: Check for NEW messages since last load
            if (Array.isArray(msgs) && prevCount > 0 && msgs.length > prevCount) {
                ...
                incoming.forEach(inc => {
                    ...
                    // 1. App is minimized/hidden (document.hidden)
                    // 2. OR User is in a DIFFERENT channel than the message
                    // 3. OR User is in the SAME channel but has scrolled up (optional, but sticking to basics)
                    
                    if (document.hidden || msgChannelKey !== currentChannelKey) {
                        ...
                        // If user is actively looking at this chat, update read status silently
    ...
    // ... (Keep existing handlers for delete, edit, upload, etc. exactly as they were) ...
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 md:h-[calc(100vh-140px)] h-[calc(100vh-180px)] flex overflow-hidden animate-fade-in relative">
            ...
            {/* ... (Rest of JSX for Group Info Modal, Sidebar, Chat Area remains exactly the same) ... */}
            {/* I am re-using the existing render structure from previous implementation since logic was the focus */}
