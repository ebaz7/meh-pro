...
import { CalendarDays, Plus, Lock, Unlock, CheckCircle2, AlertTriangle, ListOrdered, ChevronDown } from 'lucide-react';
...
export const FiscalYearSelector: React.FC = () => {
    ...
    const handleSelect = async (yearId: string) => {
        ...
        window.location.reload(); // رفرش کل سیستم برای اعمال فیلتر
    };
    ...
    return (
        <div className="relative no-print">
            ...
            >
                ...
                <span>{activeYear ? `سال ${activeYear.label}` : 'انتخاب سال مالی'}</span>
            ...
            {isOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-gray-200 z-[9999] overflow-hidden animate-scale-in">
                    <div className="p-2 bg-gray-50 border-b text-[10px] font-bold text-gray-400 uppercase tracking-tighter">سال‌های مالی تعریف شده</div>
...
// --- کامپوننت اصلی مدیریت سال مالی (برای بخش تنظیمات) ---
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100 shadow-inner">
                <h3 className="font-bold text-emerald-800 mb-4 flex items-center gap-2"><Plus size={20}/> افتتاح سال مالی جدید و تنظیم شروع شماره‌ها</h3>
                ...
                <button onClick={handleAddYear} className="w-full mt-4 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/20">
                    <CheckCircle2 size={20}/> تایید و افتتاح سال مالی
                </button>
            </div>

            <div className="space-y-3">
                <h3 className="font-bold text-gray-800 border-b pb-2 flex items-center gap-2"><CalendarDays size={20}/> تاریخچه سال‌های مالی</h3>
                {settings.fiscalYears?.map(y => (
